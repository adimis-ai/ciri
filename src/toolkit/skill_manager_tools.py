...
class SkillMetadata(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Skill identifier (max 64 chars, lowercase alphanumeric and hyphens)",
    )
    description: str = Field(
        ..., description="Description of what the skill does (max 1024 chars)"
    )
    ...
    allowed_tools: List[str] = Field(
        default_factory=list, description="Pre-approved tools list"
    )
...
class SkillInput(BaseModel):
    ...
    name: str = Field(
        ..., description="Name/identifier of the skill", min_length=1, max_length=64
    )
    description: str = Field(
        ...,
    ...
    content: str = Field(
        ..., description="Main markdown content of the skill instructions"
    )
    license: Optional[str] = Field(None, description="License name or reference")
    compatibility: Optional[str] = Field(None, description="Environment requirements")
    metadata: Dict[str, str] = Field(
    ...
    target_directory: Optional[str] = Field(
        None, description="Specific .ciri/skills directory to use (if multiple exist)"
    )
    ...
    def validate_name(cls, v):
        ...
        if not re.match(r"^[a-z0-9]+(-[a-z0-9]+)*$", v):
            raise ValueError(
                "Skill name must be lowercase alphanumeric with single hyphens between segments"
            )
        return v
...
class SkillIdentifier(BaseModel):
    ...
    target_directory: Optional[str] = Field(
        None, description="Specific .ciri/skills directory to search in"
    )
...
class SkillManager:
    ...
    def _find_skill_path(
        self, name: str, target_directory: Optional[str] = None
    ) -> Optional[Path]:
    ...
    def _parse_skill_frontmatter(self, content: str) -> tuple[SkillMetadata, str]:
        ...
        if not match:
            raise ValueError(
                "SKILL.md file must have YAML frontmatter delimited by ---"
            )
    ...
    def _create_skill_content(self, skill_input: SkillInput) -> str:
        ...
        frontmatter = {"name": skill_input.name, "description": skill_input.description}
        ...
        try:
            ...
            if skill_input.target_directory:
                ...
                if target_path in skill_dirs:
                    ...
                    raise ValueError(
                        f"Specified target directory not found: {skill_input.target_directory}"
                    )
            else:
            ...
            return {"success": False, "message": f"Failed to upsert skill: {str(e)}"}

    def delete_skill(self, identifier: SkillIdentifier) -> Dict[str, Any]:
        """Delete a skill."""
        try:
            skill_path = self._find_skill_path(
                identifier.name, identifier.target_directory
            )
            ...
            return {"success": False, "message": f"Failed to delete skill: {str(e)}"}
    ...
    def get_skill_info(self, identifier: SkillIdentifier) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "skill": skill_info.model_dump()}

        except Exception as e:
            logger.error(f"Error getting skill info for '{identifier.name}': {e}")
            return {"success": False, "message": f"Failed to get skill info: {str(e)}"}
...
def get_skill_manager() -> SkillManager:
    ...
    if _skill_manager is None:
        raise RuntimeError(
            "SkillManager not initialized. Call initialize_skill_manager first."
        )
