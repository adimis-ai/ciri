...
async def test_subagents_dynamic_scanning(project_root):
    ...
    with patch("src.middlewares.subagents.BaseSubAgentMiddleware.__init__", return_value=None) as mock_base_init:
    ...
    with patch("src.middlewares.subagents.BaseSubAgentMiddleware.awrap_model_call", new_callable=AsyncMock) as mock_base_awrap:
        class MockRequest:
            tools = []
        request = MockRequest()
