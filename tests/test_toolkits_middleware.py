...
def project_root(tmp_path):
    ...
    tk1.mkdir()
    (tk1 / "pyproject.toml").write_text('["project"]\nname="tk1"\nversion="0.1.0"\ndependencies=["fastmcp"]')
    (tk1 / "src").mkdir()
...
async def test_toolkits_dynamic_scanning(project_root):
    ...
    with patch("subprocess.run") as mock_run, \
        ...
        mock_client.get_tools = MagicMock(side_effect=lambda: asyncio.sleep(0, result=[mock_tool]))
        ...
        tk2.mkdir()
        (tk2 / "pyproject.toml").write_text('["project"]\nname="tk2"\nversion="0.1.0"\ndependencies=["fastmcp"]')
        (tk2 / "src").mkdir()
        ...
        mock_client.get_tools = MagicMock(side_effect=lambda: asyncio.sleep(0, result=[mock_tool, mock_tool2]))
