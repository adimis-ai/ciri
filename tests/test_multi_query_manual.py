...
async def test_multi_query_tool():
    ...
    with patch("src.toolkit.human_follow_up_tool.interrupt", return_value=["Answer 1", "Answer 2"]) as mock_interrupt:
        queries = [
...
async def test_cli_handling():
    ...
    with patch("src.__main__.pt_prompt", side_effect=["1", "Answer 2"]) as mock_prompt, \
